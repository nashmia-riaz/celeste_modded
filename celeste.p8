...
player = 
{
	...
	update=function(this)
		...
		local r_input = btn(k_right) or stat(28,7)
		local l_input = btn(k_left) or stat(28,4)
		local input = r_input and 1 or (l_input and -1 or 0)
		...
		local jump = stat(28, 14)  and not this.p_jump
		this.p_jump = stat(28, 14)
		if (jump) then
			this.jbuffer=4
		...
		local dash = stat(28, 15) and not this.p_dash
		this.p_dash = stat(28, 15)
   ...
   init_object(smoke,this.x,this.y)
  	...
  	if abs(this.spd.y) <= 0.15 then
   	gravity*=0.5
			...
			local up_input = btn(k_up) or stat(28,26)
			local down_input = btn(k_down) or stat(28,22)
			local v_input=(up_input and -1 or (down_input and 1 or 0))
		 	if input!=0 then
		  	if v_input!=0 then
...
function restart_room()
	will_restart=true
	delay_restart=15
	if(got_fruit[1+level_index()]) then
		score -= 100
    	got_fruit[1+level_index()] = false
	end
end
...
function _update()
	...
	-- start game
	if is_title() then
		if not start_game and (stat(28,14) or stat(28,15)) then
			music(-1)
			start_game_flash=50
			start_game=true
			sfx(38)
...
function draw_custom_ui()
	...
	if(room.x < 6) then
		rectfill(50, 116, 80, 126, 0) 
		print("level " ..room.x + 1, 52, 119, 7) 
    end
    camera(cam_x, cam_y)
end
...
function draw_score(x, y)
    -- Clears a small box for the text
    rectfill(x, y, x + 32, y + 6, 0) 
    
    -- Prints the score at the specified x and y coordinates (color 7 is white)
    print("score:" ..score, x + 1, y + 1, 7) 
end
-- helper functions --
----------------------
...

...

